...
public class MemberDAO {
	...
	public MemberDAO() {
		//MemberDAO 객체 생성시
		//member-sql.xml 파일의 내용을 읽어와 Properties 객체 생성
	...
	public int duplicateCheck(Connection conn, String memberId) throws Exception {
		...
		try {
			...
			//3) PreparesStatement 객체 생성(Connection, SQL 필요)
	...
	public int signUp(Connection conn, Member signUpMember) throws Exception {
		...
		try {
			...
			// 2) PreparesStatement 객체 생성(Connection, SQL 필요)
			pstmt = conn.prepareStatement(sql);
	...
	public List<Member> selectAll(Connection conn) throws Exception {
		...
		try {
			...
			//3) sql(SELECT) 수행 후 결과(ResultSet) 반환받기
			rs = stmt.executeQuery(sql);
			
			//4) ResultSet 한 행씩 접근(rs.next())하여 조회된 컬럼 값을 얻어와 
			...
			//6) 사용된 JDBC 자원 반환(Connection 제외)
			close(rs);
			close(stmt);
		}
	 ...
	 * @throws Exception
	 */
	public int updateMyInfo(Member updateMember, Connection conn) throws Exception {
		int result = 0;
