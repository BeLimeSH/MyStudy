...
//			+ 트랜잭션 제어 처리
//			-> 하나의 Service 메서드에서 n개의 DAO 메서드를 호출하 수 있음
//				-> n개의 DAO에서 수행된 SQL을 한 번에 commit/rollback

//	*** Service에서 트랜잭션을 처리하시 위해서는 Connection 객체가 필요하다 ***
...
public class MemberService {
	...
	public int duplicateCheck(String memberId) throws Exception {
		...
		// 2. DAO 메서드(SELECT) 호출 후 결과 반환 받고
		int result = dao.duplicateCheck(conn, memberId);
										//SQL 수행하려면 Connection이 필요하기 때문에 같이 전달
	 ...
	 * @throws Exception
	 */
	public int signUp(Member signUpMember) throws Exception { //모든예외는 View에 모아 처리
	...
	public List<Member> selectAll() throws Exception {
		...
		//2) DAO메서드(SELECT 수행) 호출 후 결과 반환 받기
	...
	public int updateMyInfo(Member updateMember) throws Exception {
		...
		int result = dao.updateMyInfo(updateMember, conn);
