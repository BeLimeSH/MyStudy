...
public class MemberView { //회원 관련 화면 입/출력
	...
	public void signUp() {	
		...
		try {
			...
			// 비밀번호, 비밀번호 확인을 각각 입력 받아 일치할 때까지 무한 반복
			while(true) {
			...
			//	-> DML 구문 수행시 성공한 행의 개수가 반환됨 == int형 변수로 결과를 저장
	...
	public Member login() {
		...
		try {
			...
			} else { //로그인 실패(아이디 또는 비밀번호 불일치 | 탈퇴한 회원)
