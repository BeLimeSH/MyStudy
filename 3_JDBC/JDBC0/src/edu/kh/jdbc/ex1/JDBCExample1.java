...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		//JDBC 객체 참조 변수 선언 (java.sql 패키지)
		
		Connection conn = null;
		//DB 연결 정보를 담은 객체
		//-> Java와 DB 사이를 연결해주는 일종의 통로(Stream과 비슷)
		...
		ResultSet rs = null;
		// SELECT 질의 성공시 반환되는 결과 행의 집합(Result Set)을 나타내는 객체
		...
		try {
			...
			//		  -> JDBC 드라이버를 통해 Connection 객체를 만드는 역할
			
			String type = "jdbc:oracle:thin:@"; //JDBC드라이버가 thin 타입
			String ip = "115.90.212.22"; //DB 서버 컴퓨터 IP
			String port = ":20000"; //:port
			String sid = ":xe"; //:DB 이름
			String user = "ksh"; //사용자 명
			String pw = "ksh1234"; //비밀번호
			...
			//중간 확인
			System.out.println(conn); //oracle.jdbc.driver.T4CConnection@5b94b04d
			...
			while(rs.next()) {
				...
				//			  순서대로 한 행씩 이동하며 다음 행이 있을 경우 true 반환
			...
			//ex) 생성 순서 : Connection, Statement, ResultSet
