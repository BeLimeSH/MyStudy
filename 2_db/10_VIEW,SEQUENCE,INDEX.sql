...
-- 별칭이 적용된 VIEW에서 직급이 '대리'인 직권만 조회
...
-- 뷰를 이용한 INSERT
INSERT INTO V_DCOPY2 VALUES('D0', 'L3'); -- 1행이 삽입되었습니다.
...
-- ORA-01400: NULL을 ("KSH"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다
...
WITH READ ONLY; -- 읽기 전용의 VIEW 생성 (DML 불가)
...
--     매번 시퀀스를 생성해서 반환하는 것보다 DB속도가 향상됨.
    ...
    ** 시퀀스 사용 방법 **
    ...
    1) 시퀀스명.NEXTVAL : 다음 시쿼스 번호를 얻어옴 (INCREMENT BY만큼 증가된 값)
                          단, 시퀀스 생성 후 첫 호출인 경우 START WITH의 값을 얻어옴.
    
    2) 시퀀스명.CURRVAL : 현재 시퀀스 번호 얻어옴.
                          단, 시퀀스 생성후 NEXTVAL 호출 없이 CURRVAL를 호출하면 오류 발생.
*/
...
ALTER TABLE EMPLOYEE_CODY4 
ADD PRIMARY KEY(EMP_ID);

SELECT * FROM EMPLOYEE_CODY4;

-- 223부터 5씩 증가하는 시퀀스 생성
...
-- 실제 사용
INSERT INTO EMPLOYEE_CODY4 VALUES(SEQ_EMP_ID.NEXTVAL, '홍길동');
INSERT INTO EMPLOYEE_CODY4 VALUES(SEQ_EMP_ID.NEXTVAL, '고길동');
INSERT INTO EMPLOYEE_CODY4 VALUES(SEQ_EMP_ID.NEXTVAL, '장길산');
-- 243
...
* 인덱스 장점
 ...
 - 조회 시 전체 테이블 내용을 조회하는 것이 아닌 인덱스가 지정된 컬럼만을
   이용해서 조회하기 때문에 시스템 부하가 낮아져 전체적인 성능이 향상된다.
 
* 인덱스 단점
 - 데이터 변경(INSERT, UPDATE, DELETE) 작업이 빈번한 경우 오히려 성능이 저하됨.
 - 인덱스도 하나의 객체이다보니 이를 저장하기 위한 별도 공간이 필요.
 - 인덱스 생성 시간이 필요
...
ON EMPLOYEE_CODY4 (EMP_NAME);


SELECT * FROM EMPLOYEE_CODY4; --> 인덱스를 사용하지 않은 검색
...
-- TB_IDX_TEST 테이블에 샘플데이터 50만개 삽입 (PL/SQL 사용)
BEGIN
    FOR I IN 1..500000
    LOOP 
...
WHERE TEST_ID = 'TEST500000'; -- 0.2??

-- 인덱스 사용 O
SELECT * FROM TB_IDX_TEST
WHERE TEST_NO = 500000; -- 0.016
