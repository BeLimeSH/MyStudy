...
SELECT 123.456, ROUND(123.456, 1) FROM DUAL; --소수점 첫째 자리까지 출력 == 소수점 둘째 자리에서 반올림
...
FROM EMPLOYEE;
 ...
 SELECT LAST_DAY('2022/04/01') FROM DUAL;SELECT LAST_DAY(SYSDATE) FROM DUAL;
...
FROM EMPLOYEE;
--> 년, 월, 일은 오라클에 등록된 날짜표기 패턴이 아니라서 오류
--> 기존에 없던 패턴 추가 시 ""로 감싸줘서 문자열 그대로를 출력하게 함.
...
SELECT EMP_NAME, JOB_CODE, TO_CHAR(((SALARY + SALARY*NVL(BONUS, 0)) * 12) + 57000000, 'L999,999,999')
FROM EMPLOYEE;
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D6'
ORDER BY JOB_CODE; -- 정렬 기준이 되는 컬럼은 반드시 SELECT절에 포함될 필요 없음.
...
FROM EMPLOYEE;
-- 그룹 함수는 여러 대를 동시 작성 가능한데 이 때 결과는 실제 테이블 한 행이 아니도
-- 각 그룹 함수별 독립된 결과
