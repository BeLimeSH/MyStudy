...
-- DEPARTMENT 테이블의 국내영업부, 마케팅부, 해외영업3부 포함
...
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE);
-- (EMPLOYEE + DEPARTMENT) JOIN LOCTION

-- 오라클 전용
SELECT EMP_NAME, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE, DEPARTMENT, LOCATION
WHERE DEPT_CODE = DEPT_ID -- EMPLOYEE, DEPARTMENT 조인
AND LOCATION_ID = LOCAL_CODE; -- (EMPLOYEE + DEPARTMENT) JOIN LOCTION

-- 조인 순서를 지키지 않은 경우(에러발생)
SELECT EMP_NAME, DEPT_TITLE, LOCAL_NAME 
FROM EMPLOYEE -- EMPLOYEE에는 LOCATION_ID가 없다.
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
...
AND E.JOB_CODE = 'J6' AND LOCATION_ID IN('L1', 'L2', 'L3');
...
-- SELF JOIN 사용
SELECT E.EMP_NAME, E.DEPT_CODE, S.EMP_NAME 동료이름
FROM EMPLOYEE E
JOIN EMPLOYEE S ON(E.DEPT_CODE = S.DEPT_CODE)
WHERE E.EMP_NAME != S.EMP_NAME
ORDER BY E.EMP_NAME;
