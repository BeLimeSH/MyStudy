...
/* TRANSACTION이란?
 ...
 - 트래잭션의 대상이 되는 데이터 변경 사항 : INSERT, UPDATE, DELETE(DML)
 
 EX) INSERT 수행 -----------------------------------> DB 반영 (X)

     INSERT 수행 ---> 트랜잭션에 추가 ---> COMMIT ---> DB 반영 (O)

     INSERT 수행 --> 1개 트랜잭션에 10개 추가 --> ROLLBACK --> DB 반영 안 됨


    1) COMMIT : 메모리 버퍼(트랜잭션)에 임시 저장된 데이터 변경 사항을 DB에 반영
    
    2) ROLLBACK : 메모리 버퍼(트래잭션)에 임시 저장된 데이터 변경 사항을 삭제하고
                  마지막 COMMIT 상태로 돌아감.
    
    3) SAVEPOINT : 메모리버퍼에 저장 지점을 정의하여 ROLLBACK 수행 시 전체 작업을
                   삭제하는 것이 아닌 저장 지점까지만 일부 ROLLBACK
    ...
    SAVEPOINT 포인트명2;
    ...
    ROLLBACK TO 포인트명1; -- 포인트1 지점까지 데이터 변견사항 삭제
