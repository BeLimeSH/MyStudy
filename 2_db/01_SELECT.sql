...
--            디벨로퍼의 날짜 표기 방법이 년/월/일 로 지정되어 있음.)
...
-- 2) DISTINCT는 SELECT문의 가장 앞에 작성 되어야 한다.

-- EMPLOYEE 테이블에 저장 된 직원들이 속해 있는 부서 코드 종류 조회
...
/* BETWEEN을 이용한 날짜 비교 */
-- EMPLOYEE 테이블에서
-- 입사일이 90/01/01 ~ 99/12/31 (90년도 입사자)
-- 직원의 사번, 이름, 입사일 조회
...
/* '1990/01/01' 날짜를 문자열 형식으로 작성하게 되면
   DB가 알아서 판단하여 날짜타입(DATE)으로 형변환을 진행 함.
*/
...
'_' 예시
1) 'A_' : A 뒤에 아무거나 한 글자
2) '___A' : A 앞에 아무거나 세 글자 (4 글자 문자열이면서 A로 끝)
*/
...
-- 문제점 : 와일드카드 문자(_)와 패턴에 사용된 일반 문자(_) 모양이 같아서 문제 발생
--> 해결 방법 : ESCAPE OPTION을 이용하여 일반 문자로 처리할'_', '%' 앞에 
--             아무 특수문자나 붙인다,
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 부서코드가 D1 또는 D6 또는 D9인
-- 사원의 사번, 이름, 부서코드 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
-- 사번이 200, 205, 210가 아닌 사원의 사번, 이름 조회
SELECT EMP_ID, EMP_NAME
FROM EMPLOYEE
...
SELECT EMP_NAME || '의 급여는 ' || SALARY || '원 입니다.' 결과
FROM EMPLOYEE;
...
/* 정렬 중첨 (큰 분류를 먼저 정렬하고 내부에 작은 분류를 정렬)*/
-- EMPLOYEE 테이블에서 부서코드 오름차순 정렬
-- 부서별 급여 내림차순 정렬
