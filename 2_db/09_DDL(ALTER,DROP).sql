-- DDL(Data Definition Languager) : 데이터 정의 언어로
...
-- 제약조건 추가 : ADD TABLE 테이블명 ADD
--                [CONSTRAINT 제약조건명] 제약조건(컬럼명) [REFERENCES 테이블명(컬럼명)]
...
--> NOT NULL은 기존 컬럼의 성질을 변경하는 것으로 인식됨(MODITY).
ALTER TABLE DEPT_COPY
...
-- ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C009401; -- NOT NULL
...
-- 컬럼 수정 : ALTER TABLE 테이블명 MODIFY 컬럼명 데이터타입; (테이터 타입 변경)
--            ALTER TABLE 테이블명 MODIFY 컬럼명 DEFAULT '값'; (기본값 변경)

--> ** 데이터 타입 수정시 컬럼에 저장된 데이터 크기 미만으로 변경할 수 없다.
...
--(수정)
-- DEPT_CAPY 테이블의 DEPT_ID 컬럼의 데이터 타입을 VARCHAR2(3)으로 변경
-- ALTER TABLE 테이블명 MODIFY 컬럼명 데이터타입;
ALTER TABLE DEPT_COPY
MODIFY DEPT_ID VARCHAR2(3);

--(수정 에러 상황)
-- DEPT_TITLE 컬럼의 데이터 타입을 VARCHAR(10)으로 변경
SELECT * FROM DEPT_COPY; -- 인사관리부 == 15BYTE
...
ALTER TABLE DEPT_COPY DROP(CANME);
