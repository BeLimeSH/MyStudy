...
// ** 주의사항 **
//고전/표준 이벤트 모델은 랜더링된 HTML 요소에 이벤트에 관련된 동작을 부여하는 코드
//-> 랜더링이 되지않은 요소에는 이벤트 관련 동작을 추가할 수 없다.
//   (문제원인 : HTML 코드 해석 순서 위->아래)

// 해결방법 : HTML요소가 먼저 랜더링 된 후 JS코드 수행
...
//고전 이벤트 모델 단점
//-> 한 요소에 동일한 이벤트 리스너(onclick)에 대한 다수의 이벤트 핸들러(배경색 변경, 폰트크기 변경)를 작성할 수 없다.
...
//change 이벤트
// - text 관련 input 태그는 입력된 값이 변할 때를 change라고 하지 않고
//   입력이 완료된 후 포커스를 잃거나, 엔터를 입력하는 경우
//   입력된 값이 이전 값과 다를 경우를 change 이벤트가 발생한 것으로 인식한다.


// a태그 기본 이벤트 제거
document.getElementById("moveNaver").addEventListener("click", function(event) {

    //매개변수 e 또는 event == 이벤트와 관련된 정보가 담겨있는 객체
    event.preventDefault(); //HTML 요소가 가지고 있는 기본 이벤트를 막음(제거)
...
document.getElementById("testBtn1").addEventListener("click", function(){
    ...
    if(in1 == "제출"){
        ...
        //document.form 태그 name 속성 값

        //form 요소.submit() : form요소 제출 함수
...
function checkIn2(){
    ...
    //#in2에 :제출이 입력된 경우에만 submit(return true)
