...
public class ArrayExample1 {
	...
	public void ex1() {
		...
		//그 공간에 arr이라는 이름을 부여
		// ** 해당 변수는 참조형을 주소 값(4byte)만을 저장할 수 있음.
	...
	public void ex3() {
		...
		//height에 heap영역에 새로 생성된 double 5칸 짜리 double[]의 시작 주소를 대입
	...
	public void ex4() {
		...
		//배열 선언 및 할당(할당할 배열의 크기는 입력 받은 크기 만큼(= input))
		...
		for(int i=0 ; i<score.length ; i++) {
			...
			sum += score[i]; //점수를 입력 받자 마자 sum 누적
		...
		int max = score[0];
		int min = score[0];
		//아래 for문을 이용해서 score 배열에 있는 모든 닶과 max. min을 비교
		//이 때, score[i] 값이 max보다 크면 max에 대입
	...
	public void ex5() {
		...
		//heap 영역에 char[]을 생성 후
		//각각 'A', 'B', 'C', 'D', 'E'로 초기화된 주소를 arr3를 대입
	...
	public void ex9() {
		...
		//3) String.charAt(index) : 문자열에서 특정 index에 위치한 문자 하나를 읽어옴.
		//  ex) "Hello".charAt(1) -> 'e'
